...
def index():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash('Falou pouco, mas falou bosta')
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Suas informações foram atualizadas')
...
def follow(username):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Agora você segue {}'.format(username))
        return redirect(url_for('user', username=username))
    else:
        return redirect(url_for('index'))
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        flash('As orientações para redefinição de senha foram enviadas em seu email')
        return redirect(url_for('login'))
    return render_template('reset_password_request.html', title='Redefinir senha', form=form)
...
def reset_password(token):
    ...
    if form.validate_on_submit():
        user.set_password(form.password.data)
        db.session.commit()
        flash('Você redefiniu a sua senha.')
        return redirect(url_for('login'))
